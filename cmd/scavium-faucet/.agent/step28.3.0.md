# Step 28.3.0 — Phase 28 Admin Config and Budget Editing API

## Goal

Add protected admin APIs for safe dynamic budget/config edits.
...
- Add admin-protected endpoints for the minimal runtime policy subset from Step 28.2.
- Validate strongly at request, domain, and persistence layers.
- Audit-log every change with before/after summaries that avoid secrets.
- Preserve current public config/status behavior with backward-compatible additions only.
- Add tests for auth, validation, audit persistence, restart behavior, and rollback/default behavior.
...
Recommended branch name: `phase-28.3-28-admin-config-budget-editing-api`.
