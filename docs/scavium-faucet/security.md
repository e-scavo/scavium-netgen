...
The export deliberately avoids unbounded or sensitive labels. It never emits wallet addresses, raw IP addresses, fingerprints, request bodies, captcha tokens, idempotency keys, private keys, admin tokens, RPC credentials, or authorization headers. Token labels are limited to sanitized runtime buckets and a bounded in-process overflow bucket. Rejections caused by invalid token IDs are aggregated into the `invalid` bucket, and early service/unavailable errors are kept in `default` rather than preserving untrusted requested token IDs.
