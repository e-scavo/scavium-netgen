...
Status: implemented and closure-audited through fix 9 post-Copilot final documentation audit.
...
- Docs, OpenAPI, tests, and SQLite migration updated; SQLite startup migrations are now defensive against partially applied `ALTER TABLE ... ADD COLUMN` migrations on production databases missing historical `schema_migrations` rows.
- Phase 30 production migration helper and runbook added for replacing the live binary with verified pre-backup, VPS-side env/nginx config candidates, optional explicit config apply, smoke validation, journal-assisted failure diagnosis, and config plus symlink/direct-binary rollback guardrails.
