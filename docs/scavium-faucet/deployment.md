...
| `../../scripts/migrate-scavium-faucet-phase30.sh` | Phase 30 production migration helper that stages through a user-writable temp dir, executes privileged VPS changes through `REMOTE_SUDO`, generates env/nginx candidate files on the VPS, optionally applies them with explicit flags, verifies pre-migration backup, archives the previous direct binary when needed, supports release-symlink and legacy direct-binary layouts, runs smoke checks from the VPS through the configured nginx/TLS URL with separate public/admin timeouts, and rolls back config plus symlink/binary on failure |
...
2. Review and fill the environment example with real values **outside the repository**. Existing production env files are not overwritten unless `APPLY_ENV_CANDIDATE=yes` is set for the Phase 30 migrator after reviewing the generated candidate.
3. Review and render the systemd and nginx templates with your final paths and domain only for first install or intentional config replacement. The Phase 30 binary migrator stages an nginx candidate and can apply it only with `APPLY_NGINX_CANDIDATE=yes`; it does not install the systemd template.
