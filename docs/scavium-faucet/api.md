...
If `SCAVIUM_FAUCET_WALLET_ALLOWED_ORIGINS` is configured, browser requests to issue wallet challenges and browser claim requests that include wallet proof fields must match one of the configured exact origins. Missing `Origin` is allowed so native wallets, CLI clients, and server-to-server integrations continue to work. Legacy claim requests without wallet proof fields continue through the normal claim path. Origin validation is defense-in-depth only and is not a replacement for the signed challenge.
