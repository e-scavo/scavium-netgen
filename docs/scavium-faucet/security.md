...
Phase 27 expands the risk engine without changing the public claim contract. The risk decision now includes an internal manual-review hint for operator diagnostics; that hint is persisted as a bounded `manual_review` abuse signal, while public callers still receive only normalized errors such as `claim_rejected`. Existing progressive IP/address/fingerprint abuse thresholds remain immediate rejection thresholds, preserving the Phase 15-26 runtime contract. The optional honeypot field is disabled by default and uses a compatibility-safe request body key (`website`); legacy clients that omit it continue to work. Burst, rotating-IP, and address-cluster heuristics are conservative score inputs backed by existing persisted `abuse_signals`, with burst detection counting successful, failed, throttled, rejected, and budget-denied intake records. Old databases remain compatible and no migration is required.
