...
- `scripts/migrate-scavium-faucet-phase30.sh` stages a new binary through a user-writable remote temp dir, executes privileged VPS operations through `REMOTE_SUDO`, preserves live env/nginx/systemd configuration while printing an advisory Phase 30 config audit, verifies a pre-migration SQLite/config backup, archives the previous direct binary in the backup bundle for legacy layouts, supports release-symlink and legacy direct-binary layouts, performs VPS-side smoke checks through `SMOKE_BASE_URL` (default `https://DEPLOY_HOST`) with separate public/admin timeouts, prints activation-window service journal excerpts on smoke failure, and restores the previous symlink or direct binary if validation fails.
