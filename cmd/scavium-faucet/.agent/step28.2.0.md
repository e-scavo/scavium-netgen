# Step 28.2.0 — Phase 28 Durable Runtime Policy Store

## Goal

Add durable runtime policy storage for a minimal safe subset.
...
- Add migrations/store methods for runtime-editable policy values only.
- Preserve existing environment config as the default/source of initial values.
- Define precedence between env config and persisted runtime overrides.
- Add tests for migration idempotency, default behavior, corrupted/invalid persisted values, and restart persistence.
...
Recommended branch name: `phase-28.2-28-durable-runtime-policy-store`.
