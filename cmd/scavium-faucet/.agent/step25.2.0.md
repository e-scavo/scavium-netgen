# Step 25.2.0 — Phase 25 Address History Endpoint

## Goal

Implement the backward-compatible public address history endpoint if it is still absent.
...
- Add `GET /api/v1/address/{address}/history` with bounded pagination.
- Reuse existing persisted claim/token metadata instead of creating duplicate state.
- Validate and normalize address input defensively below the HTTP layer.
- Return stable JSON with deterministic ordering and no secrets/internal-only fields.
- Add tests for invalid address, empty history, bounded pagination, token-aware metadata, and existing claim compatibility.
- Update `docs/scavium-faucet/api.md` and related docs.
...
Recommended branch name: `phase-25.2-25-address-history-endpoint`.
