...
- Set `SCAVIUM_FAUCET_WALLET_ALLOWED_ORIGINS` only when browser-based wallet integrations should be restricted to known application origins. It is defense-in-depth and never replaces wallet signature verification; native clients without `Origin` remain compatible. The extra gate applies to wallet challenge issuance and to claim requests that include `wallet_challenge_id` or `wallet_signature`; legacy claim requests without wallet proof fields remain accepted through the normal faucet/CORS policy.
