...
- `scripts/migrate-scavium-faucet-phase30.sh` stages a new binary through a user-writable remote temp dir, executes privileged VPS operations through `REMOTE_SUDO`, verifies a pre-migration SQLite/config backup, supports release-symlink and legacy direct-binary layouts, performs VPS-side smoke checks through `SMOKE_BASE_URL` (default `https://DEPLOY_HOST`), and restores the previous symlink or direct binary if validation fails.
