...
- Claim result and history explorer actions render only when both an absolute configured HTTP(S) explorer transaction URL and a syntactically valid transaction hash are present.
...
5. The history button rejects malformed addresses client-side, announces loading, moves focus to the history panel, and renders an empty state or recent claims for valid addresses.
6. Explorer links appear only for absolute HTTP(S) configured explorer templates containing `{txHash}` and 32-byte EVM transaction hashes.
