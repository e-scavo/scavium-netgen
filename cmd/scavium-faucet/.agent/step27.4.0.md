# Step 27.4.0 — Phase 27 Address Clustering and Optional Honeypot Challenge

## Goal

Add conservative clustering and optional honeypot/JS challenge behavior if justified by prior steps.
...
- Add address clustering only with explainable persisted relationships and no broad graph refactor.
- Optional honeypot/JS challenge must be disabled by default or harmless for existing clients unless explicitly documented as backward-compatible.
- Never make wallet/claim clients depend on fragile browser-only behavior without a compatibility path.
- Add tests for disabled default, triggered challenge path, legacy clients, and public error envelopes.
...
Recommended branch name: `phase-27.4-27-address-clustering-optional-honeypot-ch`.
