# Step 25.1.0 — Phase 25 API Inventory and Contract Gap Audit

## Goal

Audit the current public API implementation against Phase 25 before coding endpoints.
...
- Inventory current handlers/routes, public config/status/version/claim endpoints, admin endpoints, and tests.
- Confirm whether `GET /api/v1/address/{address}/history` exists, whether wallet/address status is complete, and how pagination is currently represented.
- Produce or update a Phase 25 planning note in `docs/scavium-faucet/api.md` or a dedicated Phase 25 doc only if useful.
- Do not implement new runtime behavior in this step unless a tiny documentation correction is required.
- Split any oversized implementation discovered during this audit into later `step25.*` files before coding.
...
Recommended branch name: `phase-25.1-25-api-inventory-contract-gap-audit`.
