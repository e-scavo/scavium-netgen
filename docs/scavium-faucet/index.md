...
- `scripts/migrate-scavium-faucet-phase30.sh` stages a new binary through a user-writable remote temp dir, executes privileged VPS operations through `REMOTE_SUDO`, verifies a pre-migration SQLite/config backup, activates either the release symlink or legacy direct binary path, performs local smoke checks, and restores the previous symlink/binary if validation fails.
