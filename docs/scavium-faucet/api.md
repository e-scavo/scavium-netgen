...
Explorer links in the claim result and history panels are optional. The browser renders them only when `explorer_tx_url` is configured with a `{txHash}` placeholder and the claim contains a valid 32-byte EVM transaction hash. Missing or malformed values result in no link.
