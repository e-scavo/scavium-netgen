# Step 27.2.0 — Phase 27 Risk Score Expansion

## Goal

Expand risk scoring with deterministic persisted signals and bounded metrics.
...
- Add only well-defined risk factors supported by existing or newly migrated data.
- Keep scoring deterministic, testable, and documented.
- Do not trust HTTP-only sanitization; validate/store normalized values at service/store boundaries.
- Add tests for score composition, defaults, old data compatibility, and non-leaky public responses.
- Metrics must use safe bounded labels only.
...
Recommended branch name: `phase-27.2-27-risk-score-expansion`.
