# Step 30.2.0 — Phase 30 Wallet Challenge Endpoint

## Goal

Add a wallet challenge endpoint with safe nonce/challenge issuance.
...
- Implement a wallet challenge endpoint only if absent and contractually defined by Step 30.1.
- Challenges must be short-lived, non-secret, replay-resistant, and not stored in logs as sensitive payloads if avoidable.
- Use existing storage where safe or add minimal migration with TTL/cleanup guidance.
- Add tests for challenge creation, expiry, replay resistance, invalid address, and deterministic error envelopes.
...
Recommended branch name: `phase-30.2-30-wallet-challenge-endpoint`.
