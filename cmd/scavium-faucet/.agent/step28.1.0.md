# Step 28.1.0 — Phase 28 Config and Budget Inventory

## Goal

Audit static config, budgets, admin audit logs, and persistence before dynamic editing.
...
- Inventory config loading, daily budget accounting, token budgets, admin audit, migrations, and docs.
- Define which policies are safe to mutate at runtime and which require restart.
- Do not implement mutation endpoints until safe invariants, validation, persistence, and rollback behavior are documented.
...
Recommended branch name: `phase-28.1-28-config-budget-inventory`.
