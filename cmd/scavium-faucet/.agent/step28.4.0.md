# Step 28.4.0 — Phase 28 Safe Runtime Application and Optional Allowlist Groundwork

## Goal

Apply runtime policy changes safely and lay allowlist groundwork only if explicitly scoped.
...
- Wire runtime policy reads into claim/budget decisions without data races.
- Use conservative caching or locking; prove behavior with tests.
- Add allowlist groundwork only if it is schema-safe, disabled by default, and needed for Phase 29.
- Update runbook docs for operator policy changes and rollback.
...
Recommended branch name: `phase-28.4-28-safe-runtime-application-optional-allow`.
