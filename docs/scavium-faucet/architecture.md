...
SQLite schema migrations are tracked in `schema_migrations` and applied lexically at startup. Each pending migration is executed statement-by-statement inside a transaction and is recorded only after all statements complete. `ALTER TABLE ... ADD COLUMN` duplicate-column errors are treated as already-applied column additions so production databases that were partially evolved by older binaries can finish the remaining migration statements instead of failing startup. Other migration errors remain fatal to avoid silently masking schema drift.
