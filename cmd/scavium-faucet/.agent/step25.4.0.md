# Step 25.4.0 — Phase 25 OpenAPI Contract Baseline

## Goal

Add a maintained OpenAPI contract for the stable public/admin-safe surfaces using a lightweight approach.
...
- Create or update a lightweight OpenAPI document under `docs/scavium-faucet/` without adding heavy generator dependencies.
- Cover public endpoints, normalized error envelope, headers, pagination conventions, token fields, and admin authentication notes at a contract level.
- Ensure the document matches real code and tests; do not invent endpoints.
- Add a small validation/check script only if it can be done with existing tools or standard shell checks.
- Update docs index links if a new OpenAPI file is added.
...
Recommended branch name: `phase-25.4-25-openapi-contract-baseline`.
