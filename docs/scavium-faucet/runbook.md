...
Use `scripts/migrate-scavium-faucet-phase30.sh` when replacing the current production binary with the Phase 30 build. The helper is plan-first and expects an already-reviewed local binary and supports both the release layout (`APP_PATH/current -> APP_PATH/releases/<release>`) and the current legacy/direct layout (`APP_PATH/bin/scavium-faucet`) when `current` is absent. It supports normal non-root VPS users by copying artifacts to `REMOTE_STAGE_DIR` first and executing privileged release/backup/systemd work through `REMOTE_SUDO` (`sudo` by default). It creates and verifies a remote SQLite/config backup before activation, restarts systemd, validates `/health`, `/ready`, `/api/v1/status`, `/api/v1/tokens`, and admin runtime/wallet endpoints when the token is readable, then restores the previous symlink if smoke validation fails.
